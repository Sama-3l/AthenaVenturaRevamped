...
export default function ServiceCard({id, title, subtitle, bullets, className = "sm:border-r-2"} : {id : number, title: string, subtitle : string, bullets: string[], className?: string}) {
    ...
    return (
        <div className='group w-full sm:h-[80vh] flex flex-col justify-between relative overflow-hidden'>
            ...
            <div className={`relative z-10 flex h-full flex-col sm:px-5 px-4 border-(--color-background) ${className} justify-between`}>
                <div className='flex flex-col gap-5 sm:gap-10 pb-24 sm:pb-0'>
                        ...
                        {bullets.map((item, index) => (
                            <li key={index} className={styles.service_body}>
                                <div className='flex flex-row gap-3 items-center'>
                                    <div className={`w-1.5 h-1.5 sm:w-2.5 sm:h-2.5 rounded-full shrink-0 ${styles.bullet_dot}`}/>
                                    {item}
                                </div>
                            </li>
                        ))}
            ...
            <div className='relative z-10 border-(--color-background) border-b-2 cursor-pointer'>
                <div className={`flex flex-row justify-between group-hover:border-t-(--color-foreground) border-(--color-background) border-t bg-transparent sm:border-r-2 p-4 sm:p-5 transition-colors duration-500 items-center`} onClick={() => scrollToSection("what-we-offer")}>
